...
DEFAULT_OUTPUT_DIR = ROOT / "compiled_aee_runner_target_local_11_sessions"
...
def class_signatures(state_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    before_keys = [
        "pre_build_slope", "pre_build_accel", "pre_compression_release_delta",
        "pre_macro_micro_alignment", "pre_budget_slope", "pre_noise_slope",
        "pre_exhaustion_slope", "macro_dir_score", "micro_dir_score",
        "compression", "release_quality", "remaining_budget",
    ]
    during_keys = [
        "profit_now", "mfe_so_far", "giveback_now", "velocity_now",
        "velocity_change", "progress_ratio", "energy_ratio", "noise",
        "exhaustion", "opposite_direction_strength",
    ]
...
def build_candidate_rules(base_rules: dict[str, Any], direction: str, target: str, state_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    decay_tsp = narrowed(qvals([float(r["time_since_peak"]) for r in decay_rows]), base_rules["target_modifiers"].get(target, {}).get("decay_time_since_peak", 0.0))
...
def run(input_dir: Path, output_dir: Path) -> dict[str, Any]:
    ...
    report_path = output_dir / "runner_target_local_aee_report.json"
    summary_path = output_dir / "runner_target_local_aee_summary.csv"
    classes_path = output_dir / "runner_target_local_aee_classes.json"
    trade_rows_path = output_dir / "runner_target_local_aee_trade_rows.json"
