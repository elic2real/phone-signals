...
OUT_DIR = ROOT / "compiled_aee_runner_retuned_11_sessions"
...
def main() -> None:
    ...
    for hp_add in [0.0, 0.5, 1.0, 1.5]:
        for dt_add in [0.0, 8.0, 16.0, 24.0]:
            for eb_add in [0.0, 0.06, 0.12, 0.18]:
                for hold_er_add in [0.0, 0.05, 0.10]:
                    ...
                    cand["base"]["hold"]["energy_ratio"] = round(
                        float(rules["base"]["hold"]["energy_ratio"]) + hold_er_add, 6
                    )
    ...
    (OUT_DIR / "runner_aee_report.json").write_text(json.dumps(report, indent=2))
