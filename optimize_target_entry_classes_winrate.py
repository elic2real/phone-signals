...
from optimize_target_entry_classes_contextual_v2 import pass_profile, replay_rule
...
def classify_outcomes(rows: list[dict[str, Any]], target: float) -> dict[str, Any]:
    tp_hits = 0
    sl_hits = 0
    timeouts = 0
    for row in rows:
        ...
        if abs(pips - target) < 1e-9:
            ...
            timeouts += 1
    total = len(rows)
    tp_rate = (tp_hits / total) if total else 0.0
    return {
        "trade_count": total,
        "tp_hits": tp_hits,
        "sl_hits": sl_hits,
    ...
    for row in rows:
        for rule in rules:
            if rule_applies(row, rule):
                selected.append(row)
                break
    outcome = classify_outcomes(selected, target)
    pips_per_hour = sum(float(r["static_pips"]) for r in selected) / 88.0
    expectancy = (sum(float(r["static_pips"]) for r in selected) / len(selected)) if selected else 0.0
    avg_r = expectancy / target if target else 0.0
    outcome.update(
        {
            "win_rate": outcome["tp_hit_rate"],
            "expectancy": expectancy,
            "avg_R": avg_r,
            "pips_per_hour": pips_per_hour,
        }
    )
...
def better(a: dict[str, Any], b: dict[str, Any]) -> bool:
    # Optimize TP hit rate first, then pips/hour, then trade count.
    ka = (a["tp_hit_rate"], a["pips_per_hour"], a["trade_count"])
    kb = (b["tp_hit_rate"], b["pips_per_hour"], b["trade_count"])
    return ka > kb
...
def greedy_optimize(rows: list[dict[str, Any]], candidate_rules: list[dict[str, Any]], target: float) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    while pruned and len(chosen) > 1:
        ...
        for idx in range(len(chosen)):
            ...
            if better(replay, best) or (
                replay["tp_hit_rate"] == best["tp_hit_rate"] and replay["pips_per_hour"] == best["pips_per_hour"]
            ):
                chosen = trial
                best = replay
                pruned = True
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="compiled_target_entry_classes_winrate_11_sessions", type=Path)
    args = ap.parse_args()
