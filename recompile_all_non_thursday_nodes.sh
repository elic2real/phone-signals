#!/bin/bash
# Recompile all non-Thursday nodes (Monday, Tuesday, Wednesday, Friday) with Stage 6 clustering
...
echo "=========================================="
echo "Recompiling 256 Non-Thursday Nodes"
...
for WEEKDAY in "${WEEKDAYS[@]}"; do
    for PAIR in "${PAIRS[@]}"; do
        for SESSION in "${SESSIONS[@]}"; do
            COUNT=$((COUNT + 1))
            NODE="${PAIR}__${WEEKDAY}__${SESSION}"
            LOCK="dataset_lock__$(echo $PAIR | tr '[:upper:]' '[:lower:]')__${WEEKDAY}__${SESSION}__11.json"
            ...
            if python3 run_market_node_compiler.py \
                ...
                > "/tmp/compile_${WEEKDAY}_${PAIR}_${SESSION}.log" 2>&1; then
                ...
                echo "  ❌ Failed - check /tmp/compile_${WEEKDAY}_${PAIR}_${SESSION}.log"
                FAILED=$((FAILED + 1))
            fi
            
            # Brief progress update every 10 nodes
