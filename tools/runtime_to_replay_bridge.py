...
from engine.in_between.trade_gateway_engine import build_trade_gateway_packet


DEFAULT_TRADES = Path(r"c:\Users\mawil\phone_signals\phone-signals\logs\trades.jsonl")
DEFAULT_RUNTIME_LOG = Path(r"c:\Users\mawil\phone_signals\phone-signals\logs\runtime.log")
    ...
    with trades_path.open("r", encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            if kind == "ORDER_FILLED":
                ...
                raw_db_id = str(obj.get("db_trade_id", "") or "").strip()
                db_id = raw_db_id or broker_to_db.get(br_id, br_id)
                if not db_id:
                    continue
...
def normalize_setup_name(raw_setup: Optional[str], entry_group_id: Optional[str]) -> Optional[str]:
    setup = (raw_setup or "").strip().upper()
    if not setup and entry_group_id:
        parts = str(entry_group_id).split(":")
        if len(parts) >= 2:
            setup = parts[1].strip().upper()
    if not setup:
        return None
    while setup.endswith("_RUN"):
        setup = setup[:-4]
    if setup.endswith("_MAIN"):
        setup = setup[:-5]
    return setup or None


def infer_doctrine_family(setup: Optional[str]) -> str:
    s = (setup or "").upper()
    if any(token in s for token in ("CONTINUATION", "VOL_REIGNITE", "INTENTIONAL_RUNNER", "COMPRESSION_EXPANSION")):
        return "continuation"
    if "EXHAUSTION" in s:
        return "reversal"
    if any(token in s for token in ("FAILED_BREAKOUT", "LIQUIDITY_SWEEP")):
        return "oscillation"
    return "unknown"


def build_runtime_gateway_packet(trade: RuntimeTrade) -> Dict[str, Any]:
    setup_resolved = normalize_setup_name(trade.setup, trade.entry_group_id)
    payload = {
        "source": "runtime_replay_bridge",
        "strategy_id": setup_resolved or trade.setup or trade.entry_reason or trade.trade_id,
        "setup": setup_resolved or trade.setup,
        "pair": trade.pair,
        "direction": trade.direction,
        "session": "RUNTIME_REPLAY",
        "doctrine_family_id": infer_doctrine_family(setup_resolved),
        "trade_role": trade.leg_type or "UNKNOWN",
        "horizon_class": infer_speed_class(trade),
    }
    return build_trade_gateway_packet(payload)
...
def build_bridge(
    ...
    for tr in trades:
        ...
        gateway_packet = build_runtime_gateway_packet(tr)
        entry_packet = gateway_packet.get("entry", {})
        aee_packet = gateway_packet.get("aee_handoff", {})
        ...
        manifest_rows.append(
            {
                ...
                "setup": tr.setup,
                "setup_resolved": entry_packet.get("doctrine_variant_id"),
                "setup_source": "runtime_event" if tr.setup else ("entry_group_id" if entry_packet.get("doctrine_variant_id") else "missing"),
                "doctrine_family": entry_packet.get("doctrine_family_id"),
                "speed_class": entry_packet.get("horizon_class"),
                "entry_pocket_id": entry_packet.get("entry_pocket_id"),
                "trade_role": entry_packet.get("trade_role"),
                "target_profile": entry_packet.get("target_profile"),
                "management_profile_id": aee_packet.get("management_profile_id"),
